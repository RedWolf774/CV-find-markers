"""
Тестовый файл для демонстрации выродков
"""
...
# массив с индексами выродков (находил ручками)
contour_indexes = [0, 2]

image_contours = np.uint8(np.zeros((image.shape[0], image.shape[1])))
canvas = image.copy()
for i in range(2):
    selected_contour = contours[contour_indexes[i]]
    ...
    print("Size: ", len(normalized_vector))
